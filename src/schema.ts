...
export class JazzAccount extends Account {
	...
	async migrate(this: JazzAccount) {
		...
		const { root } = await this.ensureLoaded({ root: { game: { monsters: [{}] } } });

		for (const monster of Object.values(root.game.monsters)) {
			monster.drop = EquipmentDropMap.create({}, monster._owner);
    }
	}
}
