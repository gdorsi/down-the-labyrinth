...
export default function MonstersManager() {
	...
	const handleCreate = () => {
		...
		if (
			hasEquipmentDrop &&
			selectedEquipment &&
			equipmentMap[selectedEquipment]
		) {
	...
	const startEditing = (id: string) => {
		...
		if (monster) {
			...
			if (essence) {
				...
				const essenceEffectsIds = essence.effects
					? Object.keys(essence.effects)
					: [];
							...
							<div className="space-y-4">
								...
								<div>
									...
									<select
										...
										value={type}
										onChange={(e) =>
											setType(e.target.value as "boss" | "minion")
										}
										className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
									>
											...
											effects.map((effect) => (
												<div key={effect.id} className="flex items-center mb-2">
													<input
														...
														onChange={() =>
															handleEffectToggle(effect.id, "monster")
														}
											...
											effects.map((effect) => (
												<div key={effect.id} className="flex items-center mb-2">
													<input
														...
														onChange={() =>
															handleEffectToggle(effect.id, "essence")
														}
														className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
													/>
													<label
									...
									<>
										...
										<div>
											...
											<input
												...
												onChange={(e) =>
													setEquipmentDropRate(Number(e.target.value))
												}
												className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
											/>
										</div>
